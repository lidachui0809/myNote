...
3. 栈与堆
   ...
   1. 个人总结
      栈是用来存放**函数的局部变量，以及其参数**，是低到高的向上生长的方式，所以是先今后出的存取结构，也就是先进去的地址比后进来的小，它是由系统管理并分配；

      堆内存，用于**存放变量实列对象**，是由高到低的向下生长的方式，与栈相反，所以是先进先出，有点类似于队列的性质，需用程序员自己申请开辟，以及释放，未释放其生命周期与程序进程保持一致；

      栈和堆都属于系统对进程占用的内存空间进行管理的方式，也是两种数据结构；

      ```c
      int main() //这个方法再调用前会进行压栈 这个时候a,p就会存储在栈中
      {
      	int a=0;//也就是a的地址比p的地址小
      	int *p=&a;
          return 0;
      }
      ```
